...
class GTVVideoEditActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(v: View) {
        ...
        if (id == R.id.effect_setting_btn) {
            ...
            videoEffectEditControlView!!.setVisibility(View.VISIBLE)
            val frameLayout = findViewById(R.id.glSurfaceView_placehodler) as View
        ...
        if (id == R.id.cut_video) {
            ...
            videoCutView!!.getSuitImageCount { frameCount ->
                ...
                val infos = ArrayList<GTVExtractFrameInfo>()
                //抽帧
                GTVVideoUtils.extractVideoFrame(videoPath, IGTVVideoEditor.ImageFormat.IMAGE_JPEG, FRAME_OUT_PUT_PATH, 0, mEditor!!.getDuration(), frameCount, infos)
                val urls = ArrayList<String>()
                for (info in infos) {
                    urls.add(info.getFilePath())
                }
                //设置图片路径
                videoCutView!!.setImageUrls(urls) { imgUrls, ivList ->
                    for (i in imgUrls.indices) {
                        ...
                        Glide.with(this@GTVVideoEditActivity)
            ...
            videoCutView!!.setOnVideoPlayIntervalChangeListener { startTime, endTime ->
                ...
                Log.e("GTVVideoEdit", "start = $videoIntervalStart  , end = $videoIntervalEnd")
    ...
    private fun doNext() {
        if (mEditor != null) {
            ...
            val recWorkFolder = ToolUtils.getExternalFilesPath(this@GTVVideoEditActivity) + File.separator + Constants.REC_WORK_FOLDER
            // 演示从编辑结束后画面通过系统返回键回到编辑画面
            val intent = Intent(this@GTVVideoEditActivity, RecordFinishActivity::class.java)
    ...
    override fun onBackPressed() {
        if (fromDraft == true) {
            ...
            val mAlertDialog = AlertDialog.Builder(this@GTVVideoEditActivity)
                    .setTitle("取消编辑")
                    ...
                    .setPositiveButton("是") { dialog, which ->
                        ...
                        //                        // 弹出摄像头录制画面
                        //                        Intent intent = new Intent(GTVVideoEditActivity.this, GTVVideoRecordStreamActivity.class);
                        //
                        //
                        //                        startActivity(intent);
                        //                        finish();
                        super@GTVVideoEditActivity.onBackPressed()
                    }
    ...
    private fun handleBack() {
        val intent = Intent(this@GTVVideoEditActivity, GTVVideoRecordStreamActivity::class.java)
        //String videoFile=  ToolUtils.getExternalFilesPath(PublishActivity.this) + "/draft/1/gtv.mp4";
    ...
    private fun initView() {
        ...
        mSlowMotionSeekBar!!.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
                if (mEditor != null) {
                    val duration = mEditor!!.getDuration()
    ...
    companion object {
        ...
        private val FRAME_OUT_PUT_PATH = (Environment.getExternalStorageDirectory().toString()
                + File.separator + "DCIM" + File.separator + "GTVFrameCache")//抽帧存放路径
    }
