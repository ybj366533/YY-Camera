...
import com.gtv.cloud.core.GTVVideoObject;
import com.gtv.cloud.core.IPCMAudioCallback;
import com.gtv.cloud.core.IVideoTextureOutput;
import com.gtv.cloud.impl.GTVRes;
import com.gtv.cloud.impl.recorder.GTVAudioRecorder;
import com.gtv.cloud.impl.utils.GTVMediaMuxer;
import com.gtv.cloud.impl.recorder.EGLTextureRecorder;
import com.gtv.cloud.impl.setting.AVStreamSetting;
import com.gtv.cloud.impl.encoder.HWAACEncoder;
import com.gtv.cloud.impl.encoder.HWSurfaceEncoder;
import com.gtv.cloud.impl.encoder.IAudioEncoder;
import com.gtv.cloud.impl.encoder.IVideoEncoder;
...
public class GTVVideoRecordStreamer implements IAudioEncoder.IADTCallback, IVideoEncoder.IAVCCallback, IPCMAudioCallback, IVideoTextureOutput {
    ...
    private GTVAudioRecorder GTVAudioRecorder;
    private GTVVideoObject mVideoSource;
    ...
    private GTVMediaMuxer gtvMediaMuxer;
    ...
    public GTVVideoRecordStreamer(String videoPath, AVStreamSetting setting, EGLContext context) {
    ...
    public void setInputVideoSource(GTVVideoObject vs) {
...
//    }
    ...
    public int openStream(boolean bgMusicFlag, double recordSpeed, boolean musicRecordToFile, RecordCallback callback) {
        ...
        gtvMediaMuxer = new GTVMediaMuxer();

        mQuit.set(false);

        return GTVRes.RESULT_OK;
    }
...
//            mSoundTouch.setTempo(0.5f);
        ...
        // start all
        if( GTVRes.RESULT_OK != this.mVideoEncoder.startEncoder(this.setting.getVideoWidth(), this.setting.getVideoHeight(), this.setting.getVideoBitrate(), this.setting.isOnlyIFrame())) {
            LogUtils.LOGS("initRecorder：NG ");
            return GTVRes.RESULT_ERR_PARAM_NG;
        }
        ...
        if ((this.bgMusicFlag == false) || ((this.bgMusicFlag == true) && (this.musicRecordToFile == false))) {
            ...
            this.GTVAudioRecorder = new GTVAudioRecorder(this);
            GTVAudioRecorder.startRecord();
        }
    ...
    private void destroyEncoder() {
        ...
        if (this.GTVAudioRecorder != null) {
            this.GTVAudioRecorder.stopRecord();
            this.GTVAudioRecorder = null;
        }
...
//        lastTick = tick;
    ...
    public void onAudioFrame(byte[] data, long presentationTimeUs) {
        ...
        //if(mMuxer == null) {
        if(gtvMediaMuxer.isMuxerStarted() == false) {     // 改为flag，避免在new 和 start之间 来了onAudioFrame
        ...
        if(cacheAudioNum > 0) {
            for(int i = 0; i < cacheAudioNum; ++i) {
                gtvMediaMuxer.writeAudioSampleData(cacheAudioDataList.get(i), cacheAudioTickList.get(i));
            }
        ...
        gtvMediaMuxer.writeAudioSampleData(data, ticks);
    ...
    public void onVideoMetaInfo(byte[] sps, byte[] pps) {
        ...
        if(gtvMediaMuxer.isMuxerStarted() == false) {
            if (this.mVideoEncoder.getOutputFormat() != null) {
                gtvMediaMuxer.startMuxerIfReady(videoPath, mAudioEncoder.getOutputFormat(),mVideoEncoder.getOutputFormat());
    ...
    public void onVideoFrame(byte[] videoData, int isKey, long presentationTimeUs) {
        ...
        //gtvMediaMuxer.writeVideoData(videoData, ticks, isKey);
        gtvMediaMuxer.writeVideoData(videoData, presentationTimeUs, isKey);
        //videoRecorded = true;
...
//        //callbackFreq++;
    ...
    public void onVideoFrame(ByteBuffer byteBuf, MediaCodec.BufferInfo bufferInfo) {
        ...
        gtvMediaMuxer.writeVideoData(byteBuf, bufferInfo);
    }
