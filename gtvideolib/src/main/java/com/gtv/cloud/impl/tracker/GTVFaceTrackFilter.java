...
import com.gtv.cloud.utils.GTVStickerMusicPlayer;
...
public class GTVFaceTrackFilter extends GTVImageFilter {
    ...
    private GTVFaceDetector mDetector = new GTVFaceDetector();
...
//    private boolean mFaceFlag = false;
    ...
    private GTVFaceDrawFilter debugFilter;
    private GTVDrawYuvFilter yuvFilter;
    private GTVImageFilter scaleFilter;

    private int mCacheIndex = 0;
    private GTVGroupFilter[] cacheFilters = new GTVGroupFilter[GTVDrawYuvFilter.PBO_MAX];
    private GTVGroupFilter grpFilter;
    private int[] eye_pos = new int[4];
    private GTVEyeBeautyFilter bigEyeFilter;

//    private int[] thin_face_pos = new int[8];
//    private GTVFaceSlimFilter thinFaceFilter;

    private int[] small_face_pos = new int[4];
    private GTVSmallFaceFilter smallFaceFilter;

    private GTVStickerDrawFilter stickerDrawFilter;
    ...
    public void setStickerFolder(String sf) {
        ...
        try
        {
            ...
            if(!f.exists()) {
                ...
                GTVStickerMusicPlayer.getInstance().close();
            }
            else {
                _currStickerMusic = p;
                GTVStickerMusicPlayer.getInstance().open(p);
            }
        }
        catch (Exception e)
        {
    ...
    public GTVFaceTrackFilter(EGLContext sharedContext) {
...
//        Log.d(TAG, "AFT_FSDK_GetVersion:" + version.toString() + "," + err.getCode());
        ...
        debugFilter = new GTVFaceDrawFilter();
        yuvFilter = new GTVDrawYuvFilter();
        scaleFilter = new GTVImageFilter();
        bigEyeFilter = new GTVEyeBeautyFilter();
//        thinFaceFilter = new GTVFaceSlimFilter();
        smallFaceFilter = new GTVSmallFaceFilter();
        stickerDrawFilter = new GTVStickerDrawFilter(sharedContext);
        ...
        a.add(new GTVImageFilter());
        for( int i=0; i<GTVDrawYuvFilter.PBO_MAX; i++ ) {
...
//        scaleFilter.vflip();//debug
    ...
    protected int onTrackFace(final int input_textureId) {
        ...
        this.mCacheIndex = (this.mCacheIndex + 1) % GTVDrawYuvFilter.PBO_MAX;
...
//        Log.e(TAG, "yuvFilter rotation info " + rotationInfo[0] +","+ rotationInfo[1] +","+ rotationInfo[2]);
        ...
        if( mDetector.isTrackingFace() ) {
            ...
            if( _currStickerMusic != null && pauseFlag == false) {
                GTVStickerMusicPlayer.getInstance().play();
            }
        }
        else {

            if( _currStickerMusic != null ) {
                GTVStickerMusicPlayer.getInstance().pause();
            }
        }
        /***
        ...
        if( result.size() > 0 ) {
            ...
            this.mFaceFlag = true;
            GTVEffectTracker.getInstance().updateFaceRect(this.mFaceRect.left, this.mFaceRect.top, this.mFaceRect.right-this.mFaceRect.left, this.mFaceRect.bottom-this.mFaceRect.top);
            int res = GTVEffectTracker.getInstance().trackImageJNI(smallYuvData, trackWidth*trackHeight, trackWidth, trackHeight, outputFaceData);
            if (res < 0) {
                // sdmTracker失败，目前主要原因是model还没load
                this.mFaceFlag = false;
            }

            GTVEffectTracker.getInstance().getBigEyePositionJNI(eye_pos);
            GTVEffectTracker.getInstance().getSmallFacePositionJNI(small_face_pos);
//            GTVEffectTracker.getInstance().getThinFacePositionJNI(thin_face_pos);
