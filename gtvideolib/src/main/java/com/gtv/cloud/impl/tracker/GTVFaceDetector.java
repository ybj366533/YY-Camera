...
public class GTVFaceDetector {
    ...
    public float[] imageVetexInfoBeforeDetect(int width, int height) {
        ...
        GTVEffectTracker.getInstance().getGLESImageVertex(centerX, centerY, angle, width, height, imageVertices);
...
//Log.e(TAG, "STS_FACE_GOTONE " + sdmTrackList[0] + "," + sdmTrackList[1] + "," + sdmTrackList[2]);
                ...
                int res = GTVEffectTracker.getInstance().trackImageAJNI(data, width*height, width, height, (int)center_x, (int)center_y, angle, outputFaceData);
                if (res < 0) {
                ...
                float[] t = new float[3];
                GTVEffectTracker.getInstance().getRotationParams(t);
...
/*
            ...
            int ret = GTVEffectTracker.getInstance().getEnclosingBox(box_bef);
            if( ret < 0 ) {
                try_track_ret = false;
            }
            else {
                ...
                GTVEffectTracker.getInstance().updateFaceRect(left, top, right-left, bottom-top);
                int res = GTVEffectTracker.getInstance().trackImageAJNI(data, width*height, width, height, (int)center_x, (int)center_y, angle, outputFaceData);
                if (res < 0) {
                    // sdmTracker失败，目前主要原因是model还没load
                    try_track_ret = false;
                    ...
                    ret = GTVEffectTracker.getInstance().getEnclosingBox(box_aft);
                    if( ret < 0 ) {
                        try_track_ret = false;
                    }
                    else {
