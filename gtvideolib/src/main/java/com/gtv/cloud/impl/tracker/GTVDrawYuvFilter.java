...
public class GTVDrawYuvFilter extends GTVImageFilter {
    ...
    private static class ExportShader {
        ...
        public static String getFrag(int type) {
            ...
            sb.append(HEAD);
            switch (type) {
                case GTVDrawYuvFilter.EXPORT_TYPE_I420:
                    sb.append("void main() {\n" +
                            ...
                            "}");
                    break;
                case GTVDrawYuvFilter.EXPORT_TYPE_YV12:
                    sb.append("void main() {\n" +
                            "    if(textureCoordinate.y<0.2500){\n" +
                            "        gl_FragColor=calculateY();\n" +
                            "    }else if(textureCoordinate.y<0.3125){\n" +
                            ...
                            "}");
                    break;
                case GTVDrawYuvFilter.EXPORT_TYPE_NV12:
                    sb.append("void main() {\n" +
                            "    if(textureCoordinate.y<0.2500){\n" +
                            "        gl_FragColor=calculateY();\n" +
                            "    }else if(textureCoordinate.y<0.3750){\n" +
                            ...
                            "}");
                    break;
                case GTVDrawYuvFilter.EXPORT_TYPE_NV21:
                default:
                    sb.append("void main() {\n" +
                            "    if(textureCoordinate.y<0.2500){\n" +
                            "        gl_FragColor=calculateY();\n" +
    ...
    public GTVDrawYuvFilter() {

        super(YUV_EXPORT_VERTEX_SHADER, ExportShader.getFrag(GTVDrawYuvFilter.EXPORT_TYPE_NV21));
    }
...
//    private int mRowStride;//对齐4字节
    ...
    private int bindPixelBuffer(float[] rotationInfo) {
        ...
        GTVEffectTracker.getInstance().glReadPixels(0,0, mIntputWidth, mIntputHeight * 3/ 8,GLES20.GL_RGBA,GLES20.GL_UNSIGNED_BYTE);
