...
public class GTVAnimationDrawFilter extends GTVImageFilter {

    public static final int MAX_STICKER_CNT = 5;

//    private String newStickerFolder = null;
//    private GTVEffectTracker gtvEffectTracker;

    private List<String> stickerList;
    private List<GTVEffectTracker> trackerList;
    private List<GTVAnimationTextureLoader> loadersList;
    private EGLContext mGLContext;

    public GTVAnimationDrawFilter(EGLContext sharedContext) {
    ...
    protected void onInit() {
        super.onInit();
//        gtvEffectTracker = new GTVEffectTracker();
        trackerList = new ArrayList<>();
        stickerList = new ArrayList<>();
        loadersList = new ArrayList<>();
        for( int i=0; i<MAX_STICKER_CNT; i++ ) {
            GTVEffectTracker tracker = new GTVEffectTracker();
            trackerList.add(tracker);
            GTVAnimationTextureLoader l = new GTVAnimationTextureLoader(mGLContext, tracker);
            loadersList.add(l);
            l.startLoad();
        }
    }
    ...
    protected void onDestroy() {
        ...
        for( int i=0; i<MAX_STICKER_CNT; i++ ) {
            GTVEffectTracker tracker = trackerList.get(i);
            tracker.clearTexture();
            tracker.destroy();
            GTVAnimationTextureLoader l = loadersList.get(i);
            l.stopLoad();
        }
    }
    ...
    public void setStickerFolderGroupKeepLast(List<String> g) {
        ...
        for( int i=0; i<trackerList.size(); i++ ) {
            GTVEffectTracker t = trackerList.get(i);
            t.stopPlaySticker();
        }
    ...
    public void setStickerFolder(String f) {
        ...
        if( f != null && f.length() > 0 ) {
            //GTVEffectTracker.getInstance().startPlaySticker(f);
//            gtvEffectTracker.startPlaySticker(f,0);
//            newStickerFolder = f;
        }
        else {
            //GTVEffectTracker.getInstance().stopPlaySticker();
...
//        }
        ...
        // TODO:暂时不做异步
       // GTVEffectTracker.getInstance().prepareTexture();
        //GTVEffectTracker.getInstance().udpateCanvasSize(this.getIntputWidth(), this.getIntputHeight());
        for( int i=0; i<stickerList.size() && i<trackerList.size(); i++ ) {
            GTVEffectTracker t = trackerList.get(i);
//            GTVAnimationTextureLoader loader = loadersList.get(i);
...
//        GLES20.glEnableVertexAttribArray(mGLAttribPosition);
//        GLES20.glEnableVertexAttribArray(mGLAttribTextureCoordinate);
        //GTVEffectTracker.getInstance().drawWithUniform(mGLUniformTexture, mGLAttribPosition, mGLAttribTextureCoordinate);
