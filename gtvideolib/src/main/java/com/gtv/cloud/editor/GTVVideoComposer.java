...
import com.gtv.cloud.core.GTVVideCompose;
...
public class GTVVideoComposer implements IGTVVideoComposer {
    ...
    GTVVideCompose gtvVideCompose;

    public GTVVideoComposer() {
    ...
    public void startCompose(final String filePath) {
        ...
        // 获取视频大小
        {
            ...
            int[] size = new int[2];
            GTVMP4Help.GetVideoInfo(videoPath, video_d, audio_d, size);
        ...
        gtvVideCompose = new GTVVideCompose(null);
        gtvVideCompose.setSlowMotionEnable(GTVVideoComposer.this.slowMotionEnable);
        gtvVideCompose.setSlowMotionStartTime(GTVVideoComposer.this.slowMotionStartTime);
        gtvVideCompose.setSlowMotionEndTime(GTVVideoComposer.this.slowMotionEndTime);
        gtvVideCompose.setVideoCropRange(cropRangeStartTime, cropRangeEndTime);
        gtvVideCompose.setMusicPath(GTVVideoComposer.this.musicPath, GTVVideoComposer.this.musicStartTimeMili);
        gtvVideCompose.setEffectFilterSetting(GTVVideoComposer.this.effectFilterTypeManage);
        //gtvVideCompose.setRawAudioCallback(videoRecordStreamer);
        gtvVideCompose.setOrigAudioVolume(GTVVideoComposer.this.muteFlag == true ? 0 : this.origAudioVolume);
        gtvVideCompose.setMusicVolume(this.musicVolume);
        if (logoImage != null && logoRect != null) {
            gtvVideCompose.setLogoBitmapAtRect(logoImage, logoRect);
        }

        if (logoImage2 != null && logoRect2 != null) {
            gtvVideCompose.setLogoBitmapAtRect2(logoImage2, logoRect2);
        }

        if (animationFolder != null && animationRect != null) {
            gtvVideCompose.setAnimation(animationPrefix, animationFolder, animImageInterval, animationRect);
        }


        //todo shuiin 2

        gtvVideCompose.startCompose(videoPath, filePath, width, height, new GTVVideCompose.ComposeCallback() {
            @Override
...
//                    progress *= 0.7;
//                }
                GTVVideoComposer.this.editCallback.onProgress(progress);
            }
...
//                            ret = GTVMP4Help.MP4FileTranscodeByX264(filePathT, filePath, 15, new GTVMP4Help.OnProgressListener() {
//                                @Override
//                                public int onProgress(int progress) {
//                                    //Log.e("hhhhhhhhh", " " +progress);
//                                    if (GTVVideoComposer.this.editCallback != null) {
//                                        int progress2 = 70 + (int) (progress * 0.3);
//                                        if (progress2 > 99) {
//                                            progress2 = 99;
//                                        }
//                                        GTVVideoComposer.this.editCallback.onProgress(progress2);
//                                    }
//
...
//                        }
                        ...
                        gtvVideCompose.destroy();
                        gtvVideCompose = null;

                        isCompsing = false;

                        if (GTVVideoComposer.this.editCallback != null) {
                            GTVVideoComposer.this.editCallback.onComposeFinish(ret == 0 ? 0 : 1);
                        }
    ...
    public void stopCompose() {
        ...
        if (gtvVideCompose != null) {
            gtvVideCompose.stopCompose();
        }
            ...
            if (settingObj.has("effect")) {
                ...
                if (array != null && array.length() > 0) {
                    ...
                    for (int i = 0; i < array.length(); ++i) {
                        ...
                        IGTVVideoEditor.EffectType effectType = IGTVVideoEditor.EffectType.values()[obj.getInt("effectType")];
