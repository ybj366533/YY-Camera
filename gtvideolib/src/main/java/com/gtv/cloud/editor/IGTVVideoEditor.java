...
public interface IGTVVideoEditor {
...
//    void addEffectFilter(int startTime, int endTime,EffectType effectType);
    ...
    // 获取已设置特效列表
    List<GTVideoEffectInfo> getVideoEffectList();
    void setVideoEffectList(List<GTVideoEffectInfo> list);
    ...
    void extractVideoFrame(ImageFormat imageFormat, String outDirPath, int startTime, int endTime, int dataNum, ArrayList<GTVExtractFrameInfo> frameInfoList);
    void extractAndScaleVideoFrame(ImageFormat imageFormat, String outDirPath, int startTime, int endTime, int dataNum, ArrayList<GTVExtractFrameInfo> frameInfoList, float scale);

    // webp动态图制作
    // frameInfoList 输入图片列表（视频帧抽取的YUV）
    // outputPath 动态图保存位置
    boolean createImageWebp(ArrayList<GTVExtractFrameInfo> frameInfoList, String outputPath);
