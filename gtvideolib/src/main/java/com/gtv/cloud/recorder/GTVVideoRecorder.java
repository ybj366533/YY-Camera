 ...
 * Recorder 接口实现类
 */
public class GTVVideoRecorder implements IGTVVideoRecorder {
...
//    }
    ...
    GTVVideoRecordStreamer.RecordCallback callback = new GTVVideoRecordStreamer.RecordCallback() {
        ...
        public void onProgress(int duration) {
            ...
            if (GTVVideoRecorder.this.recordCallback != null) {
                GTVVideoRecorder.this.recordCallback.onProgress(gtvVideoInfo.getTotalDuration() + duration, gtvVideoInfo);
                if ((gtvVideoInfo.getTotalDuration() + duration >= maxDuration) && (onMaxDurationSent == false)) {
                    LogUtils.LOGI(TAG, "on max duration ");
                    GTVVideoRecorder.this.recordCallback.onMaxDuration();
        ...
        public void onComplete(int duration) {
            ...
            if (duration < 100) {
                if (GTVVideoRecorder.this.recordCallback != null) {
                    GTVVideoRecorder.this.recordCallback.onRecordComplete(false, duration);
                }
            } else {
                if (GTVVideoRecorder.this.recordCallback != null) {
                    GTVVideoRecorder.this.recordCallback.onRecordComplete(true, duration);
                }

                gtvVideoSegments.addVideoSegInfo(currentRecFileName, (int) duration, (float) GTVVideoRecorder.this.recordSpeed);
...
//    }
    ...
    private void deleteLastVideoClip_impl() {
        ...
        if (GTVVideoRecorder.this.recordCallback != null) {
            GTVVideoRecorder.this.recordCallback.onProgress(gtvVideoInfo.getTotalDuration(), gtvVideoInfo);
        }
    ...
    private void startRecord_impl() {
        ...
        videoRecordStreamer = new GTVVideoRecordStreamer(recFileName, setting, gtvRecorderViewRender.getEGLContext());
    ...
    private void stopRecord_impl() {
        ...
        if (videoRecordStreamer != null) {
            ...
            if (GTVVideoRecorder.this.recordCallback != null) {
                GTVVideoRecorder.this.recordCallback.onRecordComplete(false, 0);
            }
        }
