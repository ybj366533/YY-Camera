...
import com.gtv.cloud.core.GTVCameraVideoProcessorST;
...
public class GTVVideoRecorderST implements IGTVVideoRecorderST {
    ...
    private GTVCameraVideoProcessorST gtvCameraVideoProcessorST;
...
//    }
    ...
    GTVVideoRecordStreamer.RecordCallback callback = new GTVVideoRecordStreamer.RecordCallback() {
        ...
        public void onProgress(int duration) {
            ...
            if (GTVVideoRecorderST.this.recordCallback != null) {
                GTVVideoRecorderST.this.recordCallback.onProgress(gtvVideoInfo.getTotalDuration()+duration, gtvVideoInfo);
                if ((gtvVideoInfo.getTotalDuration()+duration >= maxDuration) && (onMaxDurationSent == false)) {
                    LogUtils.LOGI(TAG, "on max duration ");
                    GTVVideoRecorderST.this.recordCallback.onMaxDuration();
        ...
        public void onComplete(int duration) {
            ...
            if (duration < 100) {
                if (GTVVideoRecorderST.this.recordCallback != null) {
                    GTVVideoRecorderST.this.recordCallback.onRecordComplete(false, duration);
                }
            } else {
                if (GTVVideoRecorderST.this.recordCallback != null) {
                    GTVVideoRecorderST.this.recordCallback.onRecordComplete(true, duration);
                }

                gtvVideoSegments.addVideoSegInfo(currentRecFileName, (int)duration, (float)GTVVideoRecorderST.this.recordSpeed);
...
//        gtvRecorderViewRender = new GTVRecorderViewRender(glSurfaceView, camId,this.recordCallback);
        ...
        gtvCameraVideoProcessorST = new GTVCameraVideoProcessorST();
...
//    }
    ...
    private void deleteLastVideoClip_impl(){
        ...
        if (GTVVideoRecorderST.this.recordCallback != null) {
            GTVVideoRecorderST.this.recordCallback.onProgress(gtvVideoInfo.getTotalDuration(), gtvVideoInfo);
        }
    ...
    public int inputVideoFrame(int textureId, int textureWidth, int textureHeight, final boolean flip, long timestampNanos){
        if(gtvCameraVideoProcessorST != null) {
            return gtvCameraVideoProcessorST.videoProcess(textureId, textureWidth, textureHeight, flip, timestampNanos);
        } else {
            return -1;
        }
    }
    ...
    public void setEGLContext(EGLContext eglContext, int textureWidth, int textureHeight){
        if(gtvCameraVideoProcessorST != null) {
            gtvCameraVideoProcessorST.setEglContext(eglContext);
        }
    }
...
//        }
        ...
        videoRecordStreamer.setInputVideoSource(gtvCameraVideoProcessorST);
    ...
    private void stopRecord_impl(){
        ...
        if (videoRecordStreamer != null) {
            ...
            if (GTVVideoRecorderST.this.recordCallback != null) {
                GTVVideoRecorderST.this.recordCallback.onRecordComplete(false, 0);
            }
        }
...
//        }
        ...
        if(gtvCameraVideoProcessorST != null) {
            gtvCameraVideoProcessorST.destroy();
        }
...
//        }
        if(gtvCameraVideoProcessorST != null) {
            gtvCameraVideoProcessorST.startAnimationGroupKeepLast(l);
        }
...
//        }
        if(gtvCameraVideoProcessorST != null) {
            gtvCameraVideoProcessorST.startAnimationGroup(l);
        }
...
//        }
        if(gtvCameraVideoProcessorST != null) {
            gtvCameraVideoProcessorST.stopAnimationGroup();
        }
