...
public class GTVVideoUtils {
    ...
    static public boolean importMovie(String fromPath, String toPath, final OnProgressCallback onProgressCallback) {
        ...
        // 只有视频转码
        int ret = GTVMP4Help.MP4FileImportVideoByX264_video(fromPath, videoPath, 1, new GTVMP4Help.OnProgressListener() {
            @Override
        ...
        if (ret != 0) {
            GTVFileUtils.deleteFile(videoPath);
            return false;
        }
        ret = GTVMP4Help.Mp4AudioVideoMerge(fromPath, videoPath, toPath);
    ...
    static public boolean importMovie(String fromPath, String toPath, int startTimeMs, int endTimeMs, final OnProgressCallback onProgressCallback ){
        ...
        int [] size = new int[2];
        GTVMP4Help.GetVideoInfo(fromPath, video_d, audio_d,size);
        if(video_d[0] <= 0) {
        ...
        if(audio_d[0] > 0) {
            ret = GTVMP4Help.MP4FileImportVideoByX264_audio(fromPath,audioPath,true,startTimeMs,endTimeMs );
            LogUtils.LOGI(TAG, "audio transcode finish: " + ret);

            if (ret != 0) {
                GTVFileUtils.deleteFile(videoPath);
                GTVFileUtils.deleteFile(audioPath);
                return false;
            }

            ret = GTVMP4Help.Mp4AudioVideoMerge(audioPath, videoPath, toPath);
        } else {
            // 如果没有音频
    ...
    // dataNum 抽取的帧数（等间隔抽取）
    // ArrayList<GTVExtractFrameInfo> frameInfoList 抽取结果（图片路径，以及对应的时间戳）
    static public void extractVideoFrame(String videoPath, IGTVVideoEditor.ImageFormat imageFormat, String outDirPath, int startTime, int endTime, int dataNum, ArrayList<GTVExtractFrameInfo> frameInfoList) {
        ...
        if (imageFormat == IGTVVideoEditor.ImageFormat.IMAGE_JPEG) {
            GTVMP4Help.extractVideoFrameToJpg(videoPath, outDirPath, startTime, endTime, dataNum, frameInfoList);
        } else if (imageFormat == IGTVVideoEditor.ImageFormat.IMAGE_YUV) {
            GTVMP4Help.extractVideoFrameToYuv(videoPath, outDirPath, startTime, endTime, dataNum, frameInfoList);
        }
...
//        }
    ...
    // outputPath 动态图保存位置
    static public boolean createImageWebp(ArrayList<GTVExtractFrameInfo> frameInfoList, String outputPath) {
