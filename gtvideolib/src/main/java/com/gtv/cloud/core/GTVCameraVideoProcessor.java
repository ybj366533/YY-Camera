...
public class GTVCameraVideoProcessor extends GTVVideoObject implements GTVBaseTexture.TextureListener, Camera.PreviewCallback {
    ...
    protected GTVFaceTrackFilter gtvStickerFilter;
    ...
    GTVRecorderViewRender gtvRecorderViewRender;
    ...
    public GTVCameraVideoProcessor(GLSurfaceView glSurfaceView, GTVRecorderViewRender gtvRecorderViewRender, int cameraId) {
        ...
        Thread b = new Thread(new Runnable() {
            @Override
            public void run() {
                GTVFileUtils.copyFileIfNeed(view.getContext(), "gtvft.bin");
                String path = GTVFileUtils.getFilePath(view.getContext(), "gtvft.bin");
                GTVEffectTracker.getInstance().loadModel(path);
            }
        });
        b.start();
    ...
    public void init() {
        ...
        //super.init(st);
        GTVBaseTexture st = new GTVSurfaceTexture();
        st.init();

        this.surfaceTexture = (GTVSurfaceTexture) st;
        ...
        try {
            ...
            CameraEngine.startPreview((SurfaceTexture) st.getTexture(), this);
        ...
        //贴纸初始化
        gtvStickerFilter = new GTVFaceTrackFilter(this.gtvRecorderViewRender.getEGLContext());
        ...
        animationDrawFilter = new GTVAnimationDrawFilter(this.gtvRecorderViewRender.getEGLContext());
...
//        }
    ...
    private void resumeCamera() {
        ...
        //}
        if (this.surfaceTexture != null) {
            CameraEngine.startPreview((SurfaceTexture) this.surfaceTexture.getTexture(), this);
        }
    }

    private void pauseCamera() {
        if (glSurfaceView != null) {
            CameraEngine.releaseCamera((Activity) GTVCameraVideoProcessor.this.glSurfaceView.getContext());
        }
    ...
    IGTVVideoRecorder.OnTextureListener onTextureListener;

    public void setTexutreListener(IGTVVideoRecorder.OnTextureListener listener) {
        this.onTextureListener = listener;
    }

    IGTVVideoRecorder.OnYUVDataListener onYUVDataListener;

    public void setYUVDataListener(IGTVVideoRecorder.OnYUVDataListener listener) {
        this.onYUVDataListener = listener;
    }

    private IGTVVideoRecorder.ITakePictureCallback mGrabBmpCB = null;

    public void takePicture(IGTVVideoRecorder.ITakePictureCallback cb) {
