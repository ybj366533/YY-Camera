...
import com.gtv.cloud.stream.GTVVideoComposeStreamer;
import com.gtv.cloud.utils.LogUtils;
import com.gtv.cloud.videoplayer.GTVideoDemuxer;
...
public class GTVVideCompose implements GTVBaseTexture.TextureListener {

    private static final String TAG = "GTVVideCompose";
...
//    }
    ...
    GTVVideoComposeVideoProessor gtvVideoComposeVideoProessor;  // 负责从surfacetexutre获取视频，特效处理，log处理 和  GTVPlayerVideoProcessor 有较高重合度。

    private long mFrameCount = 0;

    public GTVVideCompose(EGLContext context) {
...
//        openVideo(playUrl, mSf);
    ...
    GTVVideoComposeStreamer gtvVideoComposeStreamer;
    ...
    private void compose_work_thread() {
        gtVideoDemuxer = new GTVideoDemuxer(this.inVideoFilePath);
        gtVideoDemuxer.open(this.inVideoFilePath, this.mSf);
...
//            }
            gtVideoDemuxer.setRange(cropRangeStartTime, cropRangeEndTime);
        }
        ...
        gtvVideoComposeStreamer = new GTVVideoComposeStreamer(audioFilePath, videoFilePath, setting, mSharingContext);
        gtvVideoComposeStreamer.start();

        mEglCore = new EglCore(null, EglCore.FLAG_RECORDABLE);
        mInputWindowSurface = new WindowSurface(mEglCore, gtvVideoComposeStreamer.getInputSurface(), true);
        mInputWindowSurface.makeCurrent();

        gtvVideoComposeVideoProessor = new GTVVideoComposeVideoProessor(imageWidth, imageHeight);
        gtvVideoComposeVideoProessor.init();

        if (this.logoImage != null && this.logoRect != null) {
            this.gtvVideoComposeVideoProessor.loadLogoToRect(0, logoImage, logoRect);
            this.logoImage = null;
            this.logoRect = null;
        }

        if (this.logoImage2 != null && this.logoRect2 != null) {
            this.gtvVideoComposeVideoProessor.loadLogoToRect(1, logoImage2, logoRect2);
            this.logoImage = null;
            this.logoRect = null;
        }

        if (this.animationFolder != null && this.animationRect != null) {
            this.gtvVideoComposeVideoProessor.setAnimation(this.animationPrefix, this.animationFolder, this.animImageInterval, this.animationRect);
        }
        ...
        int count = 0;
        while (gtVideoDemuxer.checkEof() == 0 && composeCancelFlag == false) {
        ...
        //gtVideoDemuxer.close();
        gtvVideoComposeStreamer.closeStream();

        if (composeCancelFlag == false) {
            GTVMP4Help.Mp4AudioVideoMerge(audioFilePath, videoFilePath, outVideoFilePath);
        }

        GTVFileUtils.deleteFile(audioFilePath);
        GTVFileUtils.deleteFile(videoFilePath);

        gtvVideoComposeVideoProessor.destroy();
    ...
    private void dumuxVideoFrame() {
        ...
        if (videoIsEncoding == false) {
            int[] size = new int[2];
            int timestamp = gtVideoDemuxer.getNextVideoTimestamp(size);
            if (timestamp >= 0) {
                videoIsEncoding = true;
                gtVideoDemuxer.peekNextVideo();

                if (this.composeCallback != null) {
                    this.composeCallback.onProgress(videoPtsUs > 0 ? (int) (videoPtsUs / 1000) : 0);
                }

                //gtVideoDemuxer.removeNextVideo();
            } else {
            }
        } else {
        }
    ...
    private int demuxAudioFrame() {
        int len = gtVideoDemuxer.pullAudioData(audio_buffer);
        if (len > 0) {
            ...
            if (audio_final_buffer_short != null) {
                ...
                gtvVideoComposeStreamer.writeAudioData(audio_final_buffer_byte);
            }
        ...
        try {
            ...
            gtMusicPlayer.setPlayerEventLisenter(new GTVideoPlayer.IGTVideoPlayerListener() {
                ...
                public void onPrepared(GTVideoPlayer p) {
                    ...
                    musicPrepared = true;
                    gtMusicPlayer.seekTo((int) GTVVideCompose.this.startTimeMili);
                    gtMusicPlayer.setRange((int) GTVVideCompose.this.startTimeMili, gtMusicPlayer.getDuration() + 1000);
...
//            return;
        ...
        SurfaceTexture surfaceTexture = (SurfaceTexture) gtvSurfaceTexture.getTexture();
        this.myQueueEvent(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    int[] size = new int[2];
                    int currTimestamp = gtVideoDemuxer.getNextVideoTimestamp(size);

                    if (effectFilterTypeManage != null) {
                        //检查时间设定filter
                        IGTVVideoEditor.EffectType type = effectFilterTypeManage.getEffectFilterType(currTimestamp);
                        //if (type != magicFilterType) {
                        //Log.e("testtestFilter1", " " + getCurrentPosition() + " " + type + " " +magicFilterType);
                        gtvVideoComposeVideoProessor.setMagicFilterType(type);
                        //}
                    }

                    gtvVideoComposeVideoProessor.setDuration(currTimestamp);

                    gtvVideoComposeVideoProessor.videoProcess(0, gtvSurfaceTexture);
                    int textureid = gtvVideoComposeVideoProessor.getLastTextureId();
...
//                    }
                    ...
                    gtvVideoComposeStreamer.writeVideoData(videoPtsUs);
