...
static int muxer_convert_and_put_audio(HANDLE_GTV_DEMUXER handle, AVFrame * f)
{
    // AVSampleFormat sf;
...
static HANDLE_GTV_FFREADER open_file(char * url, ST_GTV_DATA_QUEUE* audio_queue, ST_GTV_DATA_QUEUE* video_queue)
{
    int limit = 100;
    HANDLE_GTV_FFREADER reader = gtv_ffreader_open(url, audio_queue, video_queue);
    if( reader == NULL ) {
        return NULL;
    }
    ...
    while(limit >= 0) {
        ...
        if( gtv_ffreader_check_opened(reader) > 0 ) {
            ...
            if( gtv_ffreader_audio_existed(reader) && audio_queue->count < 3 ) {
                is_audio_ready = 0;
            }
            if( gtv_ffreader_video_existed(reader) && video_queue->count < 3 ) {
                is_video_ready = 0;
            }
    ...
    if( limit < 0 && reader != NULL ) {
        GTV_ERROR("gtv_ffreader_open %s failed .", url);
...
static void close_file(HANDLE_GTV_FFREADER reader)
{
    if( reader != NULL ) {
        gtv_ffreader_close(reader);
    }
...
{
    HANDLE_GTV_DEMUXER r;
    
    if( url == NULL ) {
        GTV_ERROR("gtv_demuxer_open ng url \n");
        return NULL;
    }
    
    r = (HANDLE_GTV_DEMUXER)malloc(sizeof(ST_GTV_LIVE_DEMUXER));
    
    memset(r, 0x00, sizeof(ST_GTV_LIVE_DEMUXER));
    ...
    HANDLE_GTV_FFREADER reader = open_file(url, &(r->audio_pkt_queue), &(r->video_pkt_queue));
    if( reader == NULL ) {
        free(r);
        return NULL;
    }
...
void gtv_demuxer_close(HANDLE_GTV_DEMUXER handle)
{
    ...
    // destroy reader
    close_file((HANDLE_GTV_FFREADER)handle->reader_addr);
...
int gtv_demuxer_peek_next_video(HANDLE_GTV_DEMUXER handle, ST_GTV_RAW_FRAME_REF ref)
{
    ...
    {
        vframe = (AVFrame *)gtv_dataqueue_peek_first(&(handle->video_pkt_queue), &extra);
        if( vframe == NULL )
            break;
...
int gtv_demuxer_remove_next_video(HANDLE_GTV_DEMUXER handle)
{
    ...
    AVFrame * vframe = (AVFrame *)gtv_dataqueue_get(&(handle->video_pkt_queue), &extra);
    if( vframe == NULL )
        return 0;
...
int gtv_demuxer_pull_audio(HANDLE_GTV_DEMUXER handle, uint8_t*buf, int len)
{
    if( handle == NULL ) {
        return 0;
    }
...
int gtv_demuxer_check_eof(HANDLE_GTV_DEMUXER handle)
{
    ...
    if( handle->video_pkt_queue.count <= 0 && handle->audio_pkt_queue.count <= 0 && gtv_ffreader_check_eof(reader) > 0 ) {
        return 1;
    }
