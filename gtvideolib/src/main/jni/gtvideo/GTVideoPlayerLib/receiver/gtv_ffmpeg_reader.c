...
static int open_codec_context(HANDLE_GTV_FFREADER reader, int *stream_idx, AVCodecContext **dec_ctx, AVFormatContext *fmt_ctx, enum AVMediaType type)
{
...
{
    HANDLE_GTV_FFREADER reader = (HANDLE_GTV_FFREADER)thread_data;
    ...
    if( reader == NULL ) {
        GTV_ERROR("ffmpeg_decode_thread param null");
        return NULL;
    }
    ...
    while(reader->quit_flag == 0) {
        ...
        AVPacket * incoming_pkt = gtv_dataqueue_get(&(reader->audio_packet_decode_queue), &extra);
        ...
        if( incoming_pkt == NULL ) {
            gtv_dataqueue_wait(&(reader->audio_packet_decode_queue), 1000);
            continue;
        }
        
        // check if this is a valid pkt
        if( extra != gtv_ffreader_get_seekindex(reader) || incoming_pkt->stream_index != reader->audio_stream_idx ) {
        ...
        do {
            ...
            if (ret < 0) {
                GTV_ERROR("Error decoding audio frame (%s)\n", av_err2str(ret));
            ...
            if (got_frame > 0 && frame != NULL && reader->weak_audio_frame_queue != NULL ) {
                ...
                gtv_dataqueue_put(reader->weak_audio_frame_queue, frame, extra);
//                GTV_DEBUG("#### gtv_dataqueue_put audio:%d\n", reader->weak_audio_frame_queue->count);
            }
            else if( frame != NULL ) {
                av_frame_free(&frame);
                frame = NULL;
    ...
    // we die
    reader->audio_decode_thread_running_flag = D_GTV_COM_FLAG_OFF;
...
static void * ffmpeg_video_decode_thread(void * thread_data)
{
    ...
    while(reader->quit_flag == 0) {
        ...
        if( reader->weak_video_frame_queue != NULL && reader->weak_video_frame_queue->count > 3 && gtv_ffreader_inseeking(reader) == 0 ) {
            gtv_milliseconds_sleep(20);
            continue;
        }
        AVPacket * incoming_pkt = gtv_dataqueue_get(&(reader->video_packet_decode_queue), &extra);
        ...
        if( incoming_pkt == NULL ) {
            gtv_dataqueue_wait(&(reader->video_packet_decode_queue), 1000);
            continue;
        }
        
        // check if in seeking mode
        int seek_disp = 0;
        if( gtv_ffreader_inseeking(reader) > 0 ) {
...
//            GTV_DEBUG("#### gtv_ffreader_inseeking diff:%d, %lld\n", diff, pkt_timestamp);
        ...
        // check if this is a valid pkt
        if( (seek_disp == 0 && extra != gtv_ffreader_get_seekindex(reader)) || incoming_pkt->stream_index != reader->video_stream_idx ) {
            av_packet_free(&incoming_pkt);
            continue;
        }
        ...
        do {
            ...
            if (ret < 0) {
                GTV_ERROR("Error decoding video frame (%s)\n", av_err2str(ret));
            ...
            if (got_frame > 0 && frame != NULL && reader->weak_video_frame_queue != NULL ) {
                ...
                int rotate_degree = (int)gtv_ffreader_video_rotate_degree(reader);
                if( rotate_degree == 90 ) {
                ...
                gtv_dataqueue_put(reader->weak_video_frame_queue, frame, extra);
//                GTV_DEBUG("#### gtv_dataqueue_put video:%d\n", reader->weak_video_frame_queue->count);
            }
            else if( frame != NULL ) {
                av_frame_free(&frame);
                frame = NULL;
    ...
    // we die
    reader->video_decode_thread_running_flag = D_GTV_COM_FLAG_OFF;
...
static void * ffmpeg_read_thread(void * thread_data)
{
    ...
    if( reader == NULL ) {
        GTV_ERROR("ffmpeg_read_thread param null");
        return NULL;
    }
    ...
    // init decode queue
    gtv_dataqueue_init(&(reader->video_packet_decode_queue));
    gtv_dataqueue_init(&(reader->audio_packet_decode_queue));
    ...
    if (avformat_open_input(&(reader->fmt_ctx), reader->server_url, NULL, NULL) < 0) {
        GTV_ERROR("Could not open source file %s\n", reader->server_url);
        reader->read_error = -1;
        goto read_end;
    }
    
    if (avformat_find_stream_info((reader->fmt_ctx), NULL) < 0) {
        GTV_ERROR("Could not find stream information\n");
        reader->read_error = -2;
        goto read_end;
    }
    ...
    if (open_codec_context(reader, &(reader->video_stream_idx), &(reader->video_dec_ctx), reader->fmt_ctx, AVMEDIA_TYPE_VIDEO) >= 0) {
        ...
        reader->video_rotate_degree = gtv_ffreader_get_rotation(reader->video_stream);
    ...
    if (!reader->audio_stream && !reader->video_stream) {
        GTV_ERROR("Could not find audio or video stream in the input, aborting\n");
        reader->read_error = -3;
        goto read_end;
    }
    
    GTV_INFO("gtv-ffreader open succeed (%s) %d %d %f \n", reader->server_url, reader->width, reader->height, reader->video_rotate_degree);
    
    // launch decode thread
    if( reader->audio_stream_idx >= 0 ) {
        
        reader->audio_decode_thread_running_flag = D_GTV_COM_FLAG_ON;
        
        // start thread
        if( pthread_create(&(reader->audio_decode_thread_id), NULL, ffmpeg_audio_decode_thread, reader) < 0 )
        {
            GTV_ERROR("launch ffmpeg_audio_decode_thread failed");
            reader->audio_decode_thread_running_flag = D_GTV_COM_FLAG_OFF;
            reader->read_error = -4;
            goto read_end;
        }
    ...
    if( reader->video_stream_idx >= 0 ) {
        ...
        reader->video_decode_thread_running_flag = D_GTV_COM_FLAG_ON;
        
        // start thread
        if( pthread_create(&(reader->video_decode_thread_id), NULL, ffmpeg_video_decode_thread, reader) < 0 )
        {
            GTV_ERROR("launch ffmpeg_video_decode_thread failed");
            reader->video_decode_thread_running_flag = D_GTV_COM_FLAG_OFF;
            reader->read_error = -5;
            goto read_end;
        }
    ...
    while(reader->quit_flag == 0) {
        ...
        if( reader->order_seek_flag > 0 && reader->order_seek_timestamp >= 0 ) {
            ...
            if( reader->video_stream != NULL ) {
                ...
                k = k / 1000;
                GTV_DEBUG("seeking to %lld (%d) %f %d %d ", k, reader->order_seek_timestamp, av_q2d(reader->video_stream->time_base), reader->video_stream->time_base.den, reader->video_stream->time_base.num);
                ...
                k = k / 1000;
                GTV_DEBUG("seeking to %lld (%d) %f %d %d ", k, reader->order_seek_timestamp, av_q2d(reader->audio_stream->time_base), reader->audio_stream->time_base.den, reader->audio_stream->time_base.num);
...
//                            GTV_DEBUG("video pkt %lld --> %f \n", pkt->pts, pkt->pts * av_q2d(reader->video_stream->time_base));
                            gtv_dataqueue_put(&(reader->video_packet_decode_queue), pkt, reader->seek_index);
                            break;
                        }
                        else {
                            continue;
                        ...
                        GTV_ERROR("drop audio packet %d ", pkt->stream_index);
                        av_packet_free(&pkt);
        ...
        if( pkt->stream_index == reader->audio_stream_idx ) {
            gtv_dataqueue_put(&(reader->audio_packet_decode_queue), pkt, reader->seek_index);
        }
...
//            GTV_DEBUG("video pkt %lld --> %f \n", pkt->pts, pkt->pts * av_q2d(reader->video_stream->time_base));
            ...
            GTV_ERROR("unknow packet %d ", pkt->stream_index);
            av_packet_free(&pkt);
        }
    }
...
read_end:
    ...
    reader->seek_index = -1;
    gtv_dataqueue_put(&(reader->audio_packet_decode_queue), NULL, -9999);
    gtv_dataqueue_put(&(reader->video_packet_decode_queue), NULL, -9999);
    
    while (reader->audio_decode_thread_running_flag != D_GTV_COM_FLAG_OFF) {
        usleep(30*1000);
    }
    gtv_dataqueue_destroy(&(reader->audio_packet_decode_queue), 0);
    
    while (reader->video_decode_thread_running_flag != D_GTV_COM_FLAG_OFF) {
        usleep(30*1000);
    }
    gtv_dataqueue_destroy(&(reader->video_packet_decode_queue), 0);
    ...
    // we die
    reader->read_thread_running_flag = D_GTV_COM_FLAG_OFF;
...
{
    HANDLE_GTV_FFREADER r;
    
    if( url == NULL || strlen(url) >= GTV_COM_URL_LIMIT_LEN-1 ) {
        return NULL;
    }
    
    r = (HANDLE_GTV_FFREADER)malloc(sizeof(ST_GTV_FFMPEG_READER));
    
    memset(r, 0x00, sizeof(ST_GTV_FFMPEG_READER));
    ...
    r->read_thread_running_flag = D_GTV_COM_FLAG_ON;// 默认修改成ON，防止open完立刻调用close
    ...
    {
        GTV_ERROR("launch ffmpeg_read_thread failed");
        free(r);
        return NULL;
    }
    
    return (HANDLE_GTV_FFREADER)r;
}
...
void gtv_ffreader_close(HANDLE_GTV_FFREADER handle)
{
    ...
    while (handle->read_thread_running_flag != D_GTV_COM_FLAG_OFF) {
        usleep(30*1000);
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_get_duration null");
        return -1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_valid_range null");
        return;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_seekto null");
        return;
    }
    
    int64_t now = gtv_system_current_milli();
    ...
    handle->last_seek_milli = gtv_system_current_milli();
...
{
    if( gtv_system_current_milli() - handle->last_seek_milli < 300 ) {
        return 1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_get_seekindex null");
        return -1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_video_size null");
        return -1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_audio_existed null");
        return -1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_video_existed null");
        return -1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_check_eof null");
        return -1;
    }
...
{
    if( handle == NULL ) {
        GTV_ERROR("gtv_ffreader_video_rotate_degree null");
        return 0.0f;
    }
