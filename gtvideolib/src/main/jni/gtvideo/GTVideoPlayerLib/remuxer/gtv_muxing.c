...
typedef struct GTV_FFMUX_CORE_T {
...
{
    GTV_FFMUX_CORE * c = (GTV_FFMUX_CORE*)malloc(sizeof(GTV_FFMUX_CORE));
    
    memset((uint8_t*)c, 0x00, sizeof(GTV_FFMUX_CORE));
...
int openStream(void * handle, uint8_t * video_meta, int len)
{
    ...
    GTV_FFMUX_CORE * c = (GTV_FFMUX_CORE*)handle;
    ...
    if (!(c->ofmt_ctx)) {
        GTV_ERROR("Could not create output context(%s)\n", c->filename);
        ret = AVERROR_UNKNOWN;
        return ret;
    }
    ...
    if (!c->out_audio_stream) {
        GTV_ERROR("Could not create out_audio_stream\n");
        ret = AVERROR_UNKNOWN;
        return ret;
    }
    // add video stream
    ...
    if (!c->out_video_stream) {
        GTV_ERROR("Could not create out_video_stream\n");
        ret = AVERROR_UNKNOWN;
        return ret;
    }
    ...
    if (!(c->ofmt->flags & AVFMT_NOFILE)) {
        ...
        if (ret < 0) {
            GTV_ERROR("Could not open output file '%s'", c->filename);
            ret = AVERROR_UNKNOWN;
            return ret;
        }
    }
...
void writeAudioFrame(void * handle, uint8_t * data, int len, int64_t pts)
{
    ...
    //GTV_ERROR("@@@ av_audio : %d pts:%d %d \n", pkt.size, pkt_pts, pts);
    int ret = av_interleaved_write_frame(c->ofmt_ctx, &pkt);
    if( ret ) {
        GTV_ERROR("av_interleaved_write_frame audio failed %d\n", ret);
    }
...
void writeVideoFrame(void * handle, uint8_t * data, int len, int64_t pts, int isKey)
{
    ...
    // av_write_frame
    // GTV_ERROR("@@@ av_video : %d pts:%d %d \n", pkt.size, pkt_pts, pts);
    int ret = av_interleaved_write_frame(c->ofmt_ctx, &pkt);
    if( ret ) {
        GTV_ERROR("av_interleaved_write_frame video failed %d\n", ret);
    }
                             ...
                             enum AVCodecID codec_id)
{
    ...
    if (!st) {
        GTV_ERROR("Could not allocate stream\n");
        return NULL;
    }
