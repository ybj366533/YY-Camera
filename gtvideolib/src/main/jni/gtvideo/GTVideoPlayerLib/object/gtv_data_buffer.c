...
int gtv_databuffer_wait(ST_GTV_DATA_BUFFER * buf, int timeout)
{
...
void gtv_databuffer_dump(ST_GTV_DATA_BUFFER * buf, char * tag)
{
    int i=0;
    printf("%s $$$ ", tag);
...
int gtv_databuffer_datasize(ST_GTV_DATA_BUFFER * buf)
{
    ...
    space = gtv_databuffer_space(buf);
    s = buf->size - space - 1;
...
//        gtv_databuffer_dump(buf, "XXX");
        pthread_mutex_unlock(&buf->mutex);
        
        if( left > 0 && wait_cnt < 3 ) {
            // 尝试等待
            gtv_databuffer_wait(buf, 1000);
        }
