...
ST_GTV_RAW_FRAME_REF gtv_raw_frame_create(GTV_FRAME_TYPE type, uint64_t timestamp, int width, int height, uint8_t bgRGB[3])
{
	if (type != GTV_VIDEO_FRAME)
	{
		return NULL;
	}

    ST_GTV_RAW_FRAME_REF f = (ST_GTV_RAW_FRAME_REF)malloc(sizeof(ST_GTV_RAW_FRAME));

    if( f == NULL ) {
        return NULL;
    }

    memset(f, 0x00, sizeof(ST_GTV_RAW_FRAME));
...
ST_GTV_RAW_FRAME_REF gtv_raw_frame_create_form_data(GTV_FRAME_TYPE type, uint64_t timestamp, uint8_t * data[3], uint32_t size[3], uint32_t stride_size[3], int width, int height)
{
    int i=0;
    ST_GTV_RAW_FRAME_REF f = (ST_GTV_RAW_FRAME_REF)malloc(sizeof(ST_GTV_RAW_FRAME));
...
{
	if (frame_dst->type != GTV_VIDEO_FRAME || frame_src->type != GTV_VIDEO_FRAME)
	{
		return -1;
	}
