...
void gtvPng::Png_read_data(png_structrp png_ptr, png_bytep data, png_size_t length)
{
	gtvPng* pPng = ((gtvPng*) png_get_io_ptr(png_ptr));
...
int gtvPng::getImageFromMemory(RGBQUAD ** ppdwDst, int* pnDstW,
...
int gtvPng::getImageFromStdio(RGBQUAD ** ppdwImg, int* pnImgW, int* pnImgH,
		FILE* fp) {
	if (NULL == fp) {
...
int gtvPng::getImageFromFile(RGBQUAD ** ppdwImg, int* pnImgW, int* pnImgH,
		const char* szFile) {
