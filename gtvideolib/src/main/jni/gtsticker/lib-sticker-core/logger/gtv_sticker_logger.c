...
#include "gtv_sticker_logger.h"
#include <time.h>
#include <sys/time.h>
#include <string.h>

#if defined(TARGET_OS_IPHONE)
#include "gtv_sticker_ios_logger.h"
#endif

#if defined(TARGET_OS_ANDROID)
#include "android/gtv_sticker_plat_log.h"
#endif

#define GTV_STICKER_MAX_PRINT_LEN	2048

static uint8_t _sticker_logger_level = STICKER_LOGDEBUG;
static gtv_sticker_logger_inject_callback _gtv_sticker_logger_inj_cb = NULL;

static void gtv_sticker_logger_get_time(char* str_time, int size)
{
    //time_t now;
	struct timeval tv;
    struct tm *timenow;
...
static void gtv_sticker_log_default(int level, const char *format, va_list vl)
{
    char str[GTV_STICKER_MAX_PRINT_LEN]="";
    ...
    char str_time[32];
    gtv_sticker_logger_get_time(str_time, 32);

    snprintf(str, 32, "%s ", str_time);
    char *str2 = &str[strlen(str)];
    //vsnprintf(str, gtv_MAX_PRINT_LEN-1, format, vl);
    vsnprintf(str2, GTV_STICKER_MAX_PRINT_LEN-1 -32, format, vl);
    ////如果不是换行符结尾，则添加一个换行符
    ...
    if( _gtv_sticker_logger_inj_cb != NULL ) {
        _gtv_sticker_logger_inj_cb(str);
        return;
    }
...
void gtv_sticker_logger_print(int level, const char * format, ...)
{
    ...
    gtv_sticker_log_default(level, format, args);
    va_end(args);
}
