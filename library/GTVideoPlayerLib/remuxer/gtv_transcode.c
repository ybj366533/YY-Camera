...
typedef struct {
    ...
    ST_GTV_DATA_QUEUE video_frame_queue;
...
static void * gtv_transcode_thread(void * thread_data)
{
    int ret;

    ST_GTV_TRANSCODER* transcoder = (ST_GTV_TRANSCODER*)thread_data;
    ...
    while(1) {
        ...
        if(transcoder->video_frame_queue.count > 0) {
            int extra = 0;
            //GTV_ERROR("get frame %d \n", transcoder->video_frame_queue.count);
            AVFrame * vframe = gtv_dataqueue_get(&(transcoder->video_frame_queue), &extra);
            if(vframe != NULL) {
				if(filter_need != 0 ) {
...
int mp4_video_transcode_video(const char *in_filename, const char *out_filename, int gop_size, OnProgress onProgress, void* target, int width, int height, bool crop_flag, int start_time, int end_time)
{
    ...
    ST_GTV_TRANSCODER * transcoder = (ST_GTV_TRANSCODER *)malloc(sizeof(ST_GTV_TRANSCODER));
    memset(transcoder, 0x00, sizeof(ST_GTV_TRANSCODER));
    ...
    gtv_dataqueue_init(&(transcoder->video_frame_queue));

    if( pthread_create(&(transcoder->thread_id), NULL, gtv_transcode_thread, transcoder) < 0 )
    {
        GTV_ERROR("launch gtv_transcode_thread failed");
        free(transcoder);
        goto end;
    }
    //
    ...
    while (1) {
        ...
        if(type==AVMEDIA_TYPE_VIDEO) {
            ...
            //GTV_ERROR("packet %lld\n", packet.pts);

            ret = avcodec_send_packet(stream_ctx[0].dec_ctx, &packet);
            if (ret < 0) {
                GTV_ERROR("Error sending a packet for decoding\n");
                goto end;
            }
            ...
            while (ret >= 0) {
                ...
                gtv_dataqueue_put(&(transcoder->video_frame_queue), frame, 0);
            }
    ...
    //GTV_ERROR("%lld wait thread finish1 \n", eee_system_current_milli());
    pthread_join(transcoder->thread_id, NULL);
    //GTV_ERROR("%lld wait thread finish2  \n",eee_system_current_milli());
