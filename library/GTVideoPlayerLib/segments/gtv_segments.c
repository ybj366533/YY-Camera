...
static int _segrec_syncfile(HANDLE_GTV_SEGMENTS_REC handle)
{
    FILE * fp = NULL;
    ...
    if( fp == NULL ) {
        GTV_ERROR("open file-w err %s", handle->path);
        return 0;
    }
...
{
    FILE * fp = NULL;
    HANDLE_GTV_SEGMENTS_REC h = NULL;
    char line[D_SEG_FILE_LEN*2];
    
    if( path == NULL ) {
        return NULL;
    }
    GTV_INFO("gtv_segrec_open %s %d %d ", path, strlen(path), sizeof(ST_GTV_SEGMENTS_REC));
    
    h = (HANDLE_GTV_SEGMENTS_REC)malloc(sizeof(ST_GTV_SEGMENTS_REC));
    memset((uint8_t*)h, 0x00, sizeof(ST_GTV_SEGMENTS_REC));
    strncpy(&(h->path[0]), path, D_SEG_PATH_LEN-1);
    
    fp = fopen(path, "r");
    if( fp == NULL ) {
        // 没有现成的文件，生成一个空数据
        GTV_INFO("gtv_segrec_open first time %llx %d ", (uint64_t)h, h->count);
        return h;
    }
